...
import { doc, getDoc } from 'firebase/firestore' //  2. Getting Document Details
...
onMounted (() =>{
    ...
    console.log(auth.currentUser.uid) // 1. next to currentUser put . then 'uid' (auth.currentUser.uid) to show user id only
...
<template>
    {{ userDetails }} <!-- Checking Only For Details -->
    <!-- Cover & Basic Info -->
    ...
    <section class="bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
            <div><span class="font-medium">Major:</span> {{ userDetails?.program }}</div>
